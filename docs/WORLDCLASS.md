...
1. **Pluggable resolver / oracle interface** *(highest leverage; gates everything below)* — replace the bare-key comparison with `resolver_kind: u8 {TRUSTED_KEY, SWITCHBOARD_INDEX, OPTIMISTIC, PYTH}` + a fixed `resolver_config`. Reserve the discriminator + a padding region in `Market::SPACE` **now** so future oracle configs slot in without a layout-breaking migration. Keep `TRUSTED_KEY=0` as the clearly-labeled MVP fallback. Defer the full Switchboard/Pyth/optimistic adapters to the oracle phase. **Done:** `resolver_kind` + 64 reserved `Market` bytes; `RESOLVER_TRUSTED_KEY = 0` (default), `RESOLVER_ORACLE_FEED = 1` (first-party `PriceFeed` feed-bridge — the Switchboard On-Demand integration model), and `RESOLVER_OPTIMISTIC = 2` (UMA-style bonded assert/dispute, binary-only, settled by the guardian via `resolve_dispute`, bonds in a separate `["bond", market]` vault). **Deferred (decision recorded with evidence — [`ORACLE.md`](ORACLE.md) §11):** native Switchboard On-Demand *account parsing* in-program and a Pyth `PriceUpdateV2` kind — their SDKs resolve a conflicting `solana-program` 2.3.x against the Agave-4.0/Anchor-0.31 (`solana-program` 4.0.x) build (`AccountInfo`/type mismatch → won't typecheck; forced alignment is a dependency-hell risk); Switchboard is redundant with the feed-bridge; and Pyth carries no GPU/compute feeds (`RESEARCH.md` §6), so it is out of scope and untestable on localnet. The enum + reserved bytes keep both addable later.
...
5. **Multi-LP / LP-shares** — make `lp_shares` a real ledger: mint shares proportional to contributed liquidity *valued against current reserves* (the standard AMM pitfall — late LPs must not mint mispriced shares); add `add_liquidity`/`remove_liquidity` (pre-resolution); route a configurable fraction of taker fees to LPs pro-rata. Prerequisite for the §5 designated-MM program. **Done:** `total_shares` + a per-provider `LiquidityPosition` ledger, price-ratio-preserving `add_liquidity` / `remove_liquidity`, per-LP pro-rata `claim_pool`, all rounding favoring the pool. **Fee-to-LP routing now also done:** `config.lp_fee_bps` (set at `initialize`, updated by `set_lp_fee_bps`) routes a fee fraction to LPs by **reinvesting it as pool liquidity** — an equal `lp_cut` full set minted into both reserves (`math::lp_fee_cut`, floored), so every LP's pro-rata claim grows with no per-LP fee-debt accounting and no join-dilution; the protocol keeps `fee − lp_cut`. Conservation (`vault == collateral + fee_accrued`) is unchanged; the only side effect is a tiny documented price-nudge toward 0.5.
